...
def classify_text(self):
    ...
    script = get_hadoop_script(dfs_location, output_dfs_location, "classification.py")
    print(script)
    p = subprocess.Popen(shlex.split(script), stdout=subprocess.PIPE)
    for line in p.stdout:
        self.update_state(state="PROGRESS", meta={"status": line.rstrip().decode("utf-8")})
    subprocess.Popen(
        shlex.split("/opt/hadoop/bin/hadoop fs -get %s" % (output_dfs_location)),
...
def upload_files_dfs(self, file_location, split_size):
    ...
    for no, split in enumerate(splitted_list):
        ...
        joined = "\n".join(split.tolist())
        script = "/opt/hadoop/bin/hdfs dfs -put %s %s/%s" % (filename, dfs_location, filename)
        ...
        self.update_state(
            state="PROGRESS", meta={"status": "uploaded %s/%s" % (dfs_location, filename)}
        )
