...
def perturb_past(
    past, model, last, unpert_past=None, unpert_logits=None, accumulated_hidden=None, grad_norms=None,
    stepsize=0.01, one_hot_bows_vectors=None, classifier=None, class_label=None, loss_type=0,
    num_iterations=3, horizon_length=1, window_length=0, decay=False, gamma=1.5,
    kl_scale=0.01, device="cuda", ):
...
if __name__ == "__main__":
    ...
    add("--pretrained_model", "-M", type=str, default="gpt2-medium", help="pretrained model name or path to local checkpoint", )
    add("--cond_text", type=str, default="The lake", help="Prefix texts to condition on")
    add("--uncond", action="store_true", help="Generate from end-of-text as prefix")
    add("--num_samples", type=int, default=1, help="Number of samples to generate from the modified latents", )
    add("--bag_of_words", "-B", type=str, default=None, help="Bags of words used for PPLM-BoW. ""Either a BOW id (see list in code) or a filepath. ""Multiple BoWs separated by ;", )
    add("--discrim", "-D", type=str, default=None, choices=("clickbait", "sentiment", "toxicity", "generic"), help="Discriminator to use", )
    add("--discrim_weights", type=str, default=None, help="Weights for the generic discriminator")
    add("--discrim_meta", type=str, default=None, help="Meta information for the generic discriminator")
    add("--class_label", type=int, default=-1, help="Class label used for the discriminator", )
    add("--length", type=int, default=100)
    add("--stepsize", type=float, default=0.02)
    add("--temperature", type=float, default=1.0)
    add("--top_k", type=int, default=10)
    add("--sample", action="store_true", help="Generate from end-of-text as prefix")
    add("--num_iterations", type=int, default=3)
    add("--grad_length", type=int, default=10000)
    add("--window_length", type=int, default=0, help="Length of past which is being optimized; " "0 corresponds to infinite window length", )
    add("--horizon_length", type=int, default=1, help="Length of future to optimize over", )
    add("--decay", action="store_true", help="whether to decay or not")
    add("--gamma", type=float, default=1.5)
    add("--gm_scale", type=float, default=0.9)
    add("--kl_scale", type=float, default=0.01)
    add("--seed", type=int, default=0)
    add("--no_cuda", action="store_true", help="no cuda")
    add("--colorama", action="store_true", help="colors keywords")
    add("--repetition_penalty", type=float, default=1.0, help="Penalize repetition. More than 1.0 -> less repetition", )
